...
public:
    ...
    static QList<Track *> get_tracks_by_name(const Connector &conn, const QString name);


    static QList<Artist *> get_artists_with_filter(const Connector &conn,
                                                   bool popular_fl, bool ntrack_fl);
    static QList<Track *> get_tracks_with_filter(const Connector &conn,
