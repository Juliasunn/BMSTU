...
User *GetInfo::get_user(const Connector &conn, int id)
{
    //check user existance before!
    QString query_text = QString(QStringLiteral("select MUser.id, login, Subscribe.name, subscr_end, time_listerned"
                         " from MUser left join Subscribe "
                         " on MUser.id_subscr=Subscribe.id"
                         " where MUser.id=%1").arg(id));
...
QList<AbstractTableData *> GetInfo::get_available_subscribes(const Connector &conn, const int id_user)
{
    ...
    qDebug() << query_text;
    QList<QSqlRecord>  list_rec = conn.exec_select(query_text);
    QList<AbstractTableData *> subscribes_list;
...
QList<AbstractTableData *> GetInfo::get_playlists(const Connector &conn, const int id_user)
{
   ...
   QList<QSqlRecord>  list_rec = conn.exec_select(query_text);
   QList<AbstractTableData *> playlists_list;
...
QList<AbstractTableData *> GetInfo::get_tracks_by_artist(const Connector &conn, const int id_artist)
{
    //checked
   ...
   QList<QSqlRecord>  list_rec = conn.exec_select(query_text);
   QList<AbstractTableData *> track_list;
...
QList<AbstractTableData *> GetInfo::get_tracks_by_playlist(const Connector &conn, const int id_playlist)
{
...
QList<AbstractTableData *> GetInfo::get_artists_by_name(const Connector &conn, const QString name)
{
    //checked
   ...
   QList<QSqlRecord>  list_rec = conn.exec_select(query_text);
   QList<AbstractTableData *> artist_list;
...
QList<AbstractTableData *> GetInfo::get_tracks_by_name(const Connector &conn, const QString name)
{
    //checked
...
QList<AbstractTableData *> GetInfo::get_artists_with_filter(const Connector &conn, bool popular_fl, bool ntrack_fl)
{
    //checked
...
QList<AbstractTableData *> GetInfo::get_tracks_with_filter(const Connector &conn,
                                               bool popular_fl, bool date_fl, GenreFlags gf)
{
    //checked
    QString find_filter = gf.getFilters();

    QString sort_filter = QString();
    if (popular_fl && date_fl)
        sort_filter = QString(" order by n_listerned desc, release_date desc");
    else if (popular_fl)
        sort_filter = QString(" order by n_listerned desc");
    else if (date_fl)
        sort_filter = QString(" order by release_date desc");
    ...
    QString query_text = QString(QStringLiteral("select Track.id, Track.name, Genre.name, Track.release_date, Artist.name,"
                         ...
                         " group by Track.id, Genre.name, Track.release_date, Artist.name %2").arg(find_filter).arg(sort_filter));
