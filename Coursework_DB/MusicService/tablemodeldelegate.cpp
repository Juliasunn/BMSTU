...
void TableModelDelegate::setExtra_id(const QVariant &value)
{
    extra_id = value;
}
            ...
            for (int i = 0; i<btn_pos.size(); i++)
            {
                if (index.column() == btn_pos.at(i))
                {
                    ...
                    case DEL_FROM_PLAYLIST_SIG:
                        qDebug() << "Запрос на удаление трека " << id<< " из плейлиста"<< extra_id;
                        emit del_from_playlist(extra_id, id);
                        break;
                    ...
                    case CHOOSE_PLAYLIST_SIG:
                        qDebug() << "Запрос на добавление в плейлист"<< id << "трека" << extra_id;
                        emit choose_playlist(id, extra_id);
                        break;
                    default:
