 ...
 */
// Раго стр. 583-584
int lockfile(int fd) //передаем файловый дескриптор
{
	...
	// 	short l_whence; /*SEEK_SET - начало файла;SEEK_CUR - текущее ;SEEK_END - конец файла*/
...
int already_running(void)
{
	...
	fd = open(LOCKFILE, O_RDWR | O_CREAT, LOCKMODE); //LOCKFILE "/var/run/my_daemon.pid"
	if (fd < 0)
	{
		syslog(LOG_ERR, "невозможно открыть %s: %s", LOCKFILE, strerror(errno));
		exit(1);
	}
	if (lockfile(fd) < 0) //если блокировка уже утановлена
	{
	...
	// ftruncate - усе­ка­ет раз­мер фай­ла до указанной длины (в нашем случае до 0), так как размер предыдущей записи может быть больше той которую хотим сейчас записать (если файл уже сущ до этого)
	ftruncate(fd, 0);
